...
#'
#' @details
#' The `ExposureEffect` class manages exposure-disease relationships by:
#' \itemize{
...
#' ExposureEffect$new()$write_xps_tmplte_file("./template.csvy")
#' }
#'
#' @export
ExposureEffect <-
  R6::R6Class(
    classname = "ExposureEffect",
    ...
    public = list(
      ...
      initialize = function(
        ...
        if (file.exists(sRelativeRiskByPopulationSubsetForExposureFilePath)) {
          ...
          private$chksum <- digest(file = sRelativeRiskByPopulationSubsetForExposureFilePath)
          ...
          if ("smok_status" %in% names(dtRelativeRiskByPopulationSubset)) {
            ...
            if (
              !is.numeric(dtRelativeRiskByPopulationSubset$smok_status) &&
                !is.factor(dtRelativeRiskByPopulationSubset$smok_status)
            ) {
              stop(
                "Column 'smok_status' must be numeric or factor, but found: ",
            ...
            dtRelativeRiskByPopulationSubset[,
              smok_status := factor(
                smok_status,
                levels = 1:(max(smok_status, na.rm = TRUE))
              )
            ]
          } # end of smok_status
          if ("ethnicity" %in% names(dtRelativeRiskByPopulationSubset)) {
            ...
            unique_ethnicity <- unique(as.character(
              dtRelativeRiskByPopulationSubset$ethnicity
            ))

            # Use symmetric difference to find any mismatched levels
            mismatched_levels <- setdiff(
              union(unique_ethnicity, expected_ethnicity_levels),
            ...
            # Standardize ethnicity factor levels using helper
            private$set_ethnicity_factor_levels(
              dtRelativeRiskByPopulationSubset
            )
          } # end of ethnicity
        ...
        # Clean up old checksum-based files for this exposure~outcome
        old_files <- list.files(
          private$filedir,
          pattern = paste0("^rr_", private$suffix, "_[a-f0-9]+_(indx|l)\\.fst$"),
          full.names = TRUE
        )
        ...
        private$filenam <- file.path(
          private$filedir,
          paste0("rr_", private$suffix, "_l.fst")
        )
        private$filenam_indx <- file.path(
          private$filedir,
          paste0("rr_", private$suffix, "_indx.fst")
        )
        ...
        # Standardize factor levels using helper functions
        private$set_smoking_factor_levels(private$input_rr)
        private$set_ethnicity_factor_levels(private$input_rr)
        invisible(self)
      },
      ...
      gen_stochastic_effect = function(
        ...
        # Check if regeneration is needed based on registry and file existence
        files_exist <- file.exists(private$filenam) && file.exists(private$filenam_indx)
        registry_valid <- files_exist && private$check_registry_valid()
        
        needs_regen <- overwrite || !files_exist || !registry_valid
        
        # Also check if existing file has insufficient age range
        if (!needs_regen && files_exist) {
          needs_regen <- max(read_fst(private$filenam, columns = "age")) < design_$sim_prm$ageH
        }
        ...
        if (needs_regen) {
          ...
          # Standardize factor levels using helper functions
          private$set_smoking_factor_levels(stoch_effect)
          private$set_ethnicity_factor_levels(stoch_effect)
      ...
      del_stochastic_effect = function(invert = FALSE) {
        ...
        if (invert) {
          # With registry system, invert mode is no longer needed
          # since there are no versioned files to clean up
          message("invert=TRUE is deprecated with registry-based file versioning.")
        } else {
          # Delete files and remove from registry
          if (file.exists(private$filenam)) file.remove(private$filenam)
          if (file.exists(private$filenam_indx)) file.remove(private$filenam_indx)
          private$remove_from_registry()
        }
        invisible(self)
      },
    ...
    private = list(
      ...
      # set_ethnicity_factor_levels ----
      # Standardize Ethnicity Factor Levels
      # @description
      # Helper function to standardize ethnicity column to factor with consistent levels
      # across all methods. This ensures uniform handling of ethnicity stratification.
      #
      # @param dt A data.table containing an 'ethnicity' column to standardize.
      #
      # @return NULL. Modifies the data.table in place by reference.
      #
      # @details
      # Converts ethnicity to a factor with 9 standardized levels in fixed order:
      # white, indian, pakistani, bangladeshi, other asian, black caribbean,
      # black african, chinese, other.
      #
      # @keywords internal
      set_ethnicity_factor_levels = function(dt) {
      ...
      # set_smoking_factor_levels ----
      # Standardize Smoking Status Factor Levels
      # @description
      # Helper function to standardize smoking status column to factor with 4 levels.
      # Ensures consistent handling of smoking stratification across methods.
      #
      # @param dt A data.table containing a 'smok_status' column to standardize.
      #
      # @return NULL. Modifies the data.table in place by reference.
      #
      # @details
      # Converts smok_status to a factor with 4 levels (1-4):
      # 1 = never smoker, 2 = occasional smoker, 3 = ex-smoker, 4 = current smoker.
      #
      # @keywords internal
      set_smoking_factor_levels = function(dt) {
        if ("smok_status" %in% names(dt)) {
          dt[, smok_status := factor(smok_status, levels = 1:4)]
        }
        invisible(NULL)
      }
    ) # end of private
