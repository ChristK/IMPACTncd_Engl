---
title: "How to run a test of IMPACTncdEngland"
author: "Dr. Chris Kypridemos, Dr. Anna Head, Adithi R. Upadhya"
date: "11 March, 2024"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{How to run a test of IMPACTncdEngland}
...
Whether using IMPACTncdEngland in Rstudio or in a terminal, we need to ensure that the working directory is set to where the model is downloaded. 
...
Two main files are used to run IMPACTncdEngland: 
...
- `num_chunks` is the number of synthpops to aggregate together. This can be a smaller value for a test run eg - 2. The sample size for each simulation is n x num_chunks. Therefore, If you use n = 20,000 and num_chunks = 10, you will be simulating a cohort of 200,000 synthetic individuals (which is the default and suitable for most simulations)
...
If any of the `locality`, `n`, `num_chunks`, and `sim_horizon_max` parameters in this file are changed, it is recommended that you delete the existing output and synthpop folders and then run the code from here again.
...
#### How to remove IMPACTncdEngland installed package 
