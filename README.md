...
5. To install certain large data files, stored as GitHub repo *assets*. A design file `inputs/sim_design.yaml` holds details which ease asset installation. Open the file `inputs/sim_design.yaml` and change bOverwriteFilesOnDeploy to `FALSE` or `TRUE` depending on whether you want to overwrite existing files. uploadSrcDirectory and deployToRootDirectory (both should be the same) to some path in your system eg : *D:/IMPACTncd_Engl* OR *C:/IMPACTncdEngl/IMPACTncd_Engl*, this path should be the root directory of where your .proj file is stored as these are absolute paths. sToken should be the specific github token for this project (ensure your system is protected) or `NULL`. 
6. Then in the terminal type `Rscript global.R`. For this step to run without interruption a strong internet connection / bandwidth is required
