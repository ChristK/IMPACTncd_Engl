...
setwd("/mnt/edefoj/UoL/CPRD2025/edefoj.cprd/epi_models")

disnm <- "CHD" # disease name
disnm2 <- "chd" # disease name
overwrite_incd <- TRUE
overwrite_prvl <- TRUE
overwrite_ftlt <- TRUE
overwrite_dur  <- TRUE
...
strata <- c("year", "age", "sex", "dimd", "sha", "ethnicity", "urbanicity")
strata_ftlt <- c("year", "age", "sex", "dimd","urbanicity")
strata_dpnd <- c("age", "sex", "dimd")

source(paste0(
  "/mnt/", Sys.info()[["user"]],
  "/UoL/CPRD2025/edefoj.cprd/epi_models/scripts/aux_fn.R"))


#source ("/mnt/edefoj/UoL/CPRD2025/edefoj.cprd/epi_models/scripts/aux_fn.R")

# Duration ====
if (overwrite_dur ||
    !file.exists(output_path(paste0(disnm2, "_dur.qs")))) {
  
  dt <- harmonise(read_fst(input_path("panel_short_prev_2019_years_CY.fst"),
...
dt <- na.omit(dt)
  ...
  dur_model <- gamlss(
    dur ~ pb(age) + pcat(sex) + pcat(dimd) + pcat(ethnicity) + pcat(urbanicity),
    ~pb(age) + pcat(sex) + pcat(dimd) +  pcat(urbanicity),
    ~pb(age),
    family = "ZINBI",
    data = dt,
    method = mixed(20, 100)
  ...
  newdata <-
    CJ(
      ...
      urbanicity = levels(dt$urbanicity)
    )
...
if (overwrite_incd ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_inc_CY.fst"),
                           as.data.table = TRUE)[gender != "I"])[between(age, 20, 100) &
                                                                   get(disnm) < 2L &
                                                                   year < 2023, .SD, .SDcols = c(disnm, strata)][, .(incd = sum(get(disnm), na.rm = TRUE), n = .N),
                                                                                                                 keyby = strata]
  dt[, no_incd := n - incd]
  dt[, year := year - 2000]
  dt <- na.omit(dt)
  ...
  mod_max <- gamlss(
    y ~ I(year >= 18) + (
      log(year) + pb(age) + pcat(sex) + pcat(dimd) + pcat(sha) + pcat(ethnicity) + pcat(urbanicity)
    ) ^ 2 ,
    family = BI(),
    data = dt,
    method = mixed(20, 100)
...
if (overwrite_prvl ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_prev_CY.fst"),
                           as.data.table = TRUE)[gender != "I"])[between(age, 20, 100) &
                                                                   get(disnm) <= 2L &
                                                                   year < 2023, .SD, .SDcols = c(disnm, strata)][, .(prvl = sum(get(disnm) == 2L, na.rm = TRUE), n = .N),
                                                                                                                 keyby = strata]
  dt[, no_prvl := n - prvl]
  dt[, year := year - 2000]
  dt <- na.omit(dt)
...
if (overwrite_ftlt ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_prev_CY.fst"),
      ...
      year < 2023, .SD, .SDcols = c(disnm, strata_ftlt, "death_cause")
  ...
  mod_max <- gamlss(
    y ~ (log(year) + pb(age) + pcat(sex) + pcat(dimd) + pcat(urbanicity)) ^ 2,
    family = BI(),
    data = dt,
    method = mixed(20, 100)
  )
...
#     !file.exists(output_path(paste0("dependencies/",disnm2, "_dpndRR.qs")))) {
#   
#   print("start dpnd_all")
#   
#   dpnd <- c("Anxiety_Depression - spell", "Alcohol problems" ,"Atrial Fibrillation", "COPD"  ,
...
#   keep <- results[, max(statsig), by = dpnd_on][V1 == 1, dpnd_on]
#   
#   results <- results[dpnd_on %in% keep , .(sex, agegroup, dimd, rr , ci_rr = uci_rr, dpnd_on )]
#   dpnd2 <- dpnd2[dpnd2 %in% keep]
#   
#   
#   type <- "_prvl"
