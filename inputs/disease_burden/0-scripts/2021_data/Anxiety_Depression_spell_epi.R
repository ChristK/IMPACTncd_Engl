...
disnm <- "Anxiety_Depression - spell" # disease name
disnm2 <- "andep" #  # disease name for saving
...
if (overwrite_incd ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_inc.fst"),
                           as.data.table = TRUE)[gender != "I"])[between(age, 20, 100) &
                                                                   get(disnm) < 2L &
                                                                   year < 2020, .SD, .SDcols = c(disnm, strata)][
                                                                     , .(incd = sum(get(disnm), na.rm = TRUE), n = .N),
                                                                     keyby = strata]
  dt[, no_incd := n - incd]
  dt[, year := year - 2000]
...
if (overwrite_prvl ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_prev.fst"),
                      ...
                      year < 2020, .SD, .SDcols = c(disnm, strata)
                    ][, .(prvl = sum(get(disnm) == 2L, na.rm = TRUE), n = .N),
                      keyby = strata]
  dt[, no_prvl := n - prvl]
  dt[, year := year - 2000]
...
if (overwrite_ftlt ||
  ...
  #   method = mixed(20, 100)
  # )
  # mod_max <- stepGAIC(mod_max, direction = "backward")
  #
  ...
  # newdata2[newdata1, on = .NATURAL, mu2 := i.mu2]
  newdata2[, dimd := factor(dimd, as.character(1:10))]
  setkeyv(newdata2, strata_ftlt)
  write_fst(newdata2, output_path(paste0(disnm2, "_ftlt.fst")), 100L)
...
if (overwrite_dur ||
  ...
  #yaml::write_yaml(cc, "~/My Models/IMPACTncd_Engl/inputs/disease_burden/andep_dur_forward.yaml")
  yaml::write_yaml(cc, output_path("andep_dur_forward.yaml"))
...
if (overwrite_dpnd ||
  ...
  disnm2 <- "andep" # disease name for yaml  files


  dpnd <- c(paste0(disnm, "ever"),
            "Stroke",
            "Pain",
            "Other cancers",
  ...
  #dpnd2 <- gsub(" ", "_", dpnd) #having spaces is a problem
  #dpnd2 <- gsub("-", "", dpnd2) #having dashes is a problem
  ...
  #disnm2 <- gsub(" ", "", disnm) #having spaces is a problem
  #disnm2 <- gsub("-", "_", disnm2) #having hyphens is a problem
  disnm2 <- "andep"
  ...
  results[dpnd_on == "pastandep", dpnd_on := "andep"]
