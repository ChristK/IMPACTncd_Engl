...
disnm <- "Dementia" # disease name
disnm2 <- "dementia" # disease name
overwrite_incd <- TRUE
overwrite_prvl <- TRUE
overwrite_ftlt <- TRUE
overwrite_dur  <- TRUE
...
if (overwrite_dur ||
  ...
  #distr_nam <- names(marg_distr$fits[2]) # pick appropriately and note here ZANBI
  #ZAPIG was no1, but slower? 3 parameters + q has a max value
  #   ZAPIG    ZANBI
  #  27168.36 27227.41
...
# Dependencies ==== Leaving for now, as have to work out how to do stroke 
#in the last year for people who don't have inc dementia 
# if (overwrite_dpnd ||
#     !file.exists(output_path(paste0("dependencies/",disnm2, "_dpndRR.qs")))) {
#   
#   dpnd <- c("Post-stroke Dementia", "Stroke") #This is stroke 1 yr before dementia onset
#   #dpnd2 <- gsub(" ", "_", dpnd) #having spaces is a problem
#   #dpnd2 <- gsub("-", "_", dpnd2) #having dashes is a problem
#   dpnd2 <- "stroke"
#     
#   dt <- harmonise(read_fst(input_path("panel_short_inc.fst"),
#                            as.data.table = TRUE)[gender != "I"])[
...
#   
#   for (j in dpnd) { #Need to make the exposures binary - NB the exposure is incidence
#     set(dt, NULL, j, fifelse(dt[[j]] == 1L, 1L, 0L))
#   }
#   rm(j)
#   
#   
...
#   
#   disnm2 <- "dementia"
#   setnames(dt, c(disnm, dpnd), c(disnm2,dpnd2)) 
#   
#   results <- data.table()
