...
disnm <- "Asthma - spell" # disease name
disnm2 <- "asthma" # disease name for saving
...
# disease list "Anxiety_Depression"            "Asthma" "Atrial Fibrillation"
# "CHD" "COPD"                          "Chronic Kidney Disease" "Dementia"
# "Heart failure" "Hypertension"                  "Obesity" "Other cancers"
# "Primary Malignancy_Breast" "Primary Malignancy_Colorectal" "Primary
# Malignancy_Lung" "Primary Malignancy_Prostate"   "Stroke" "Type 2 Diabetes
# Mellitus"
...
if (overwrite_dur ||
  ...
  # marg_distr <- fitDist( dt$dur, log(nrow(dt)), type = "count", # "realplus",
  #  try.gamlss = TRUE, trace = TRUE ) head(marg_distr$fits)

  # workHORSEmisc::distr_validation(marg_distr, dt[between(dur, 0, 50), .(var =
  #                  dur, wt = 1)], expression(bold(duration ~ (years))),
  #                  discrete = TRUE)

  # distr_nam <- names(marg_distr$fits[1]) # pick appropriately and note here
  # ZANBI
  ...
  # c(0, m$mu.coefficients[grep("dimd", names(m$mu.coefficients))])[as.integer(dimd)])]
  # dt[, s2 := exp(m$sigma.coefficients["(Intercept)"] + m$sigma.coefficients["log(age)"] * log(age) + c(0, m$sigma.coefficients["sexwomen"])[as.integer(sex)] +
  #                  c(0, m$sigma.coefficients[grep("dimd", names(m$sigma.coefficients))])[as.integer(dimd)])]
  ...
  cc <- list(
    ...
    "mu" = list(
      ...
      "log(age)" = m$mu.coefficients["log(age)"],
      "sex" = c(0, m$mu.coefficients["sexwomen"]),
      "dimd" = c(0, m$mu.coefficients[grep("dimd", names(m$mu.coefficients))])
    ),
    "sigma" = list(
      "intercept" = m$sigma.coefficients["(Intercept)"],
      "log(age)" = m$sigma.coefficients["log(age)"],
      "sex" = c(0, m$sigma.coefficients["sexwomen"]),
      "dimd" = c(0, m$sigma.coefficients[grep("dimd", names(m$sigma.coefficients))])
    ),
    "nu" = list(
      "intercept" = m$nu.coefficients["(Intercept)"],
      "log(age)" = m$nu.coefficients["log(age)"],
      "sex" = c(0, m$nu.coefficients["sexwomen"]),
      "dimd" = rep(0, 10)
    )
  )

  #yaml::write_yaml(cc, "~/My Models/IMPACTncd_Engl/inputs/disease_burden/asthma_dur_forward.yaml")
  yaml::write_yaml(cc, output_path("asthma_dur_forward.yaml"))
}
...
if (overwrite_ftlt ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_prev.fst"),
    as.data.table = TRUE
  )[gender != "I"])[between(age, 20, 100) & get(disnm) > 0L &
...
if (overwrite_dpnd ||
  ...
  results[dpnd_on == "pastasthma", dpnd_on := "asthma"]
