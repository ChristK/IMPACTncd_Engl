...
disnm <- "ckd45" # disease name 
codnm <- c("Chronic Kidney Disease stage 4", "End Stage Renal Disease") #cause of death 
# This is CKD ofstages 4 & 5 
overwrite_incd <- TRUE
overwrite_prvl <- TRUE
overwrite_ftlt <- TRUE
overwrite_dur  <- TRUE
...
if (overwrite_ftlt ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_prev.fst"),
      ...
      ][, .(ftlt = sum(death_cause %in% codnm, na.rm = TRUE),
