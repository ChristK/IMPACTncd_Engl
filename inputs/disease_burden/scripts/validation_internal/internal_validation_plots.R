...
output_dir_incd <-
  function(x = character(0))
    paste0("/mnt/", Sys.info()[["user"]],
           "/UoL/CPRD2021/epi_models/scripts/validation_internal/plots_incd/", x)

output_dir_prvl <-
  function(x = character(0))
    paste0("/mnt/", Sys.info()[["user"]],
           "/UoL/CPRD2021/epi_models/scripts/validation_internal/plots_prvl/", x)

output_dir_ftlt <-
  function(x = character(0))
    paste0("/mnt/", Sys.info()[["user"]],
           "/UoL/CPRD2021/epi_models/scripts/validation_internal/plots_ftlt/", x)
...
#Adding a temporary weight, will need to delete later
out[, wt := 1] 
...
for (i in dl_cprd_new){
  ...
  ggplot(subtab1, aes(x = year, y = V1 * 100000, col = Type)) +
    ...
    expand_limits(y = 0) 
  ggsave(filename = output_dir_incd(paste0(i,".png",sep="")))
...
for (i in dl_cprd_new){
  ...
  ggsave(filename = output_dir_prvl(paste0(i,".png",sep="")))
...
ggplot(subtab1, aes(x = year, y = V1 * 100000, col = Type)) +
  ...
  scale_colour_brewer(type = "qual") 
ggsave(filename = output_dir_ftlt("all_cause_mort.png"))
...
for (i in dl_cprd_new[dl_cprd_new != "obesity"]){
  ...
  ggplot(subtab1, aes(x = year, y = V1 * 100, col = Type)) +
    ...
    expand_limits(y = 0) +
    scale_colour_brewer(type = "qual") 
  ggsave(filename = output_dir_ftlt(paste0("cf_", i,".png",sep="")))
