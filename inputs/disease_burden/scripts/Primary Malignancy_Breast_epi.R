...
strata <- c("year", "age",  "dimd", "sha", "ethnicity")
strata_ftlt <- c("year", "age", "dimd")
...
if (overwrite_incd ||
  ...
  dt <- harmonise(read_fst(input_path("panel_short_inc.fst"),
                           as.data.table = TRUE)[gender == "F"])[between(age, 20, 100) &
                                                                   get(disnm) < 2L &
  ...
  mod_max <- gamlss(
    y ~ (
      pb(year) + pb(age) + pcat(dimd) + pcat(sha) + pcat(ethnicity)
    ) ^ 2,
    family = BI(),
    data = dt,
    method = mixed(20, 100)
