...
disnm <- "Hearing loss" # disease name
disnm2 <- "helo" # disease name
overwrite_incd <- TRUE
overwrite_prvl <- TRUE
overwrite_ftlt <- TRUE
overwrite_dur  <- TRUE
...
if (overwrite_incd ||
  ...
  mod_max <- gamlss(
    y ~ (
      log(year) + I(year >= 13) + pb(age) + pcat(sex) + pcat(dimd) + pcat(sha) + pcat(ethnicity)
    ) ^ 2,
    family = BI(),
    data = dt,
    method = mixed(20, 100)
...
#Isn't any CF from hearing loss
if (overwrite_ftlt ||
